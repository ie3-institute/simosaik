  ...
  private static Map<String, Object> handleResult(ResultEntity result, List<String> attrs) {
    ...
    if (result instanceof FlexRequestResult) {
      ...
      if (attrs.contains(FLEX_OPTION_P_MIN)) {
        data.put(
            FLEX_OPTION_P_MIN,
            options.getpMin().to(PowerSystemUnits.MEGAWATT).getValue().doubleValue());
      }

      if (attrs.contains(FLEX_OPTION_P_REF)) {
        data.put(
            FLEX_OPTION_P_REF,
            options.getpRef().to(PowerSystemUnits.MEGAWATT).getValue().doubleValue());
      }

      if (attrs.contains(FLEX_OPTION_P_MAX)) {
        data.put(
            FLEX_OPTION_P_MAX,
            options.getpMax().to(PowerSystemUnits.MEGAWATT).getValue().doubleValue());
      }
      ...
      if (attrs.contains(MOSAIK_ACTIVE_POWER)) {
        Optional<Double> optional =
            setPoint
                .getSetPoint()
                .flatMap(PValue::getP)
                .map(p -> p.to(PowerSystemUnits.MEGAWATT).getValue().doubleValue());
