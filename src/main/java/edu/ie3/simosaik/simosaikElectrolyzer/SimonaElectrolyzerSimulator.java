...
public class SimonaElectrolyzerSimulator extends SimonaSimulator {
  ...
  private static final JSONObject meta =
      (JSONObject)
          JSONValue.parse(
              ("{"
                      + "    'api_version': '"
  ...
  public long step(long time, Map<String, Object> inputs, long maxAdvance) throws Exception {
    ...
    try {
      ...
      ExtInputDataContainer primaryDataForSimona =
          SimosaikUtils.createSimosaikPrimaryDataWrapper(time, inputs, nextTick);
  ...
  public void setMapping(ExtEntityMapping mapping) {
    ...
    this.simonaPrimaryEntities =
        this.mapping.getExtId2UuidMapping(EXT_INPUT).keySet().toArray(new String[0]);
    this.simonaResultEntities =
        this.mapping.getExtId2UuidMapping(EXT_RESULT_GRID).keySet().toArray(new String[0]);
  }
}
